"""Linear-specific pipeline integration test — extract → score → graph write."""
...
def _linear_raw() -> RawEvent:
    return RawEvent(
        source="linear",
        source_id="linear:comment:comment-demo-1",
        workspace_id="local-dev",
        event_type="linear:comment:create",
        content="Comment on ENG-42: We decided to adopt event sourcing for billing.",
...
def test_linear_comment_through_worker_scores_before_write(
    ...
    """Linear RawEvent runs extract → scoring pipeline → Neo4j write in order."""
    raw_event = _linear_raw()
    decision = _linear_decision(raw_event)

    def _apply_scores(d: DecisionEvent) -> DecisionEvent:
        d.importance_score = 0.79
        d.trust_score = 0.72
        return d
